...
- Annotated `vX.Y.Z` release-tag publishing to GHCR for `linux/amd64` and `linux/arm64`, gated by successful quality/container jobs and strict latest-`origin/main` target validation, with commit-addressed tags, maximum BuildKit provenance, SBOM, GitHub OIDC build provenance pushed to the registry, and secret-free digest metadata artifacts.
- Release workflow contract tests covering event routing and job dependencies, annotated tag shape and latest-main target, OIDC/package permissions, image names, platforms, attestations, and artifact fields.
