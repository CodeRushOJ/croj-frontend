...
如果浏览器仍带有过期令牌，公开题目、标签、公告、比赛、论坛和题解读取会协调为同一个会话失效事件：只清理一次失效会话、只显示一次非阻塞提示，并把各公开请求分别移除 `Authorization` 后匿名重试一次。延迟返回的旧令牌 401 不会清除刚建立的新会话；受保护路由会保留完整 `redirect` 查询后进入登录页。比赛题目仍要求登录，因为它们受比赛可见性与参赛状态约束。
