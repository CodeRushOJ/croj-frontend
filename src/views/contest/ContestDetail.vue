  ...
  <section v-if="contest" class="contest-detail">
    ...
    <div v-else-if="activeTab === 'problems'" class="panel problem-panel">
      ...
      <article v-for="(problem, index) in contestProblems" :key="problem.id" class="contest-problem">
        ...
        <div><strong>{{ problem.title }}</strong><small>{{ problem.problemNo }} · {{ problem.tags.map(tag => tag.name).join(' / ') }}</small></div>
        <span :class="`difficulty-${problem.difficulty}`">{{ difficultyLabel(problem.difficulty) }}</span>
        <router-link :to="{ name: 'ProblemDetail', params: { problemNo: problem.problemNo } }">开始解题 →</router-link>
      </article>
    </div>
    ...
    <div v-else class="panel scoreboard-panel">
      ...
      <div v-for="row in scoreboard" :key="row.rank" class="score-row"><b>{{ row.rank }}</b><strong>{{ row.username }}</strong><span>{{ row.solved }}</span><span>{{ row.penalty }}</span></div>
    </div>
  </section>
...
const scoreboard = [{ rank: 1, username: 'north_star', solved: 5, penalty: 312 }, { rank: 2, username: 'lambda', solved: 4, penalty: 247 }, { rank: 3, username: 'preview-admin', solved: 3, penalty: 198 }]
