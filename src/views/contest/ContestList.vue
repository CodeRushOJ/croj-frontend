...
.eyebrow { color: var(--accent-color); font-size: 12px; font-weight: 800; letter-spacing: .12em; text-transform: uppercase; }
...
.phase-tabs button span { margin-left: 5px; color: inherit; opacity: .65; }.phase-tabs button.active { border-color: var(--accent-color); color: var(--accent-color); background: var(--accent-color-soft); }
.contest-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 18px; }
.contest-card { min-height: 320px; padding: 22px; display: flex; flex-direction: column; border: 1px solid var(--border-color-light); border-radius: 20px; background: var(--card-bg); box-shadow: 0 12px 36px rgba(61,49,39,.055); position: relative; overflow: hidden; }
.contest-card::before { content: ''; position: absolute; inset: 0 0 auto; height: 4px; background: var(--border-color); }.contest-card--running::before, .contest-card--registration::before { background: var(--accent-color); }
.contest-card__top { display: flex; justify-content: space-between; color: var(--text-color-secondary); font-size: 12px; }.phase-badge { color: var(--accent-color); font-weight: 800; }
