<template>
  <div class="compose-page">
    ...
    <form class="compose-card" @submit.prevent="submitPost">
      ...
      <div class="field"><label for="post-category">分类</label><select id="post-category" v-model="form.category"><option v-for="category in categories" :key="category">{{ category }}</option></select></div>
      ...
      <div class="actions"><router-link :to="{ name: 'Forum' }"><el-button>取消</el-button></router-link><el-button native-type="submit" type="primary" :loading="submitting">发布讨论</el-button></div>
    </form>
  </div>
</template>
...
const form = reactive({ title: "", category: categories[0], content: "" });
const error = ref("");
...
const submitPost = async () => {
  ...
  try {
    ...
    await router.push({ name: "ForumPostDetail", params: { postId: response.data.id } });
